...
pid = fork {
  cmd = "GEM_CMD=/opt/local/bin/gem GEM_PATH=#{prefix}/vendor PATH=#{prefix}/bin:#{prefix}/vendor/bin:#{prefix}/../shared/ruby/bin:$PATH SPITBALL_CACHE=#{shared_prefix}/cache ruby -rubygems -Ilib bin/spitball-server -p 1134 >> '#{shared_prefix}/spitball.log' 2>&1"
  exec(cmd)
}
