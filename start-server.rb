...
pid = fork {
  cmd = "GEM_CMD=/opt/local/bin/gem PATH=#{prefix}/bin:#{prefix}/vendor/bin:#{prefix}/../shared/ruby/bin:$PATH SPITBALL_CACHE=#{shared_prefix}/cache ruby -Ilib -rubygems bin/spitball-server -p 1134 >> '#{shared_prefix}/spitball.log' 2>&1"
  exec(cmd)
}
